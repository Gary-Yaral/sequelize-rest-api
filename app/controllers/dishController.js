...
async function add(req, res) {
  ...
  try {
    const user = await Dish.create(req.body, {transaction})
...
async function update(req, res) {
  ...
  try { 
    ...
    await Dish.update(req.body, {where: {id: req.params.id}}, {transaction})
...
async function remove(req, res) {
  ...
  try {
    ...
    const affectedRows = await Dish.destroy({ where: { id }, transaction})
    ...
    const filterCondition = {
      ...
      where: {
        [Op.or]: [
          ...
          { '$DishType.type$': { [Op.like]: `%${filter}%` } }
        ]
      }
    }
    // Realizar la consulta con paginación y filtros
    const data = await Dish.findAndCountAll(filterCondition)
    return res.json({
      result: true,
      data
    }) 
